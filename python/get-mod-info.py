...
def get_line_from_archive(file_path: str, text_to_search: str):
...
for filename in os.listdir(DIRECTORY):
    ...
    if os.path.isfile(filepath):
        ...
        # Get the name
        text_line = get_line_from_archive(filepath, 'name')
        ...
        # Check which download mode is selected and get the URL
        url_mode = get_line_from_archive(filepath, 'mode')
        if 'metadata:curseforge' in url_mode:
            text_line = get_line_from_archive(filepath, 'project-id')
            ...
            text_line = get_line_from_archive(filepath, 'url')
            if text_line == None: continue
            text_line = trim_line(text_line)
            final_text += TEXT_SEPARATOR + text_line
...
# Write result to the text file in CSV friendly format
